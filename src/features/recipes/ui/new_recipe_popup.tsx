...
import IngredientCreateDialog from "@/features/recipes/ui/ingredient_create_dialog";
import { iIngredientRecipe } from "@/server/db/queries/getIngredients";
...
type Ingredient = NonNullable<
  createRecipeIngredienceSchema["ingredients"]
>[number];
    ...
    <form
      ...
      action={(fd) => {
        const payload: iRecipeSchema = {
          ...
          ingredients: ingredients
            .map((i) => ({
              ...i,
              name: i.name,
              recipeIngredientsId: i.recipeIngredientsId,
            }))
        ...
        startTransition(async () => {
          ...
          setMessage(
            res.ok ? (res.message ?? "Saved.") : `${res.error ?? "unknown"}`
          );
          if (res.ok) {
            // reset
            setTitle("");
            ...
            setRecipeCategory("");
            setIngredients([
              { recipeIngredientsId: 0, name: "", quantity: 0, unitId: 1 },
            ]);
            setSteps([{ text: "" }]);
          }
        });
                ...
                <IngredientCreateDialog
                  ...
                  onCreated={(row) => {
                    setIngredientOptions((prev) => {
                      if (
                        prev.some((p) => p.id === row.id || p.name === row.name)
                      )
                        return prev;
            ...
            {ingredients.map((ing, i) => (
              <div key={i} className="grid grid-cols-1 gap-2 md:grid-cols-11">
                ...
                <div className="md:col-span-6">
                  <Select
                    value={
                      ing.recipeIngredientsId
                        ? String(ing.recipeIngredientsId)
                        : undefined
                    }
                ...
                <div className="md:col-span-2">
                  <Select
                    ...
                    onValueChange={(v) =>
                      updateIngredient(i, { unitId: v ? Number(v) : undefined })
                    }
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      {initialUnits.map((unitRow) => (
                        <SelectItem key={unitRow.id} value={String(unitRow.id)}>
                          {unitRow.shortForm
                            ? `${unitRow.name} (${unitRow.shortForm})`
                            : unitRow.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
