...
import {CreateRecipeRowSchema, RecipeSchema} from "@/zodSchemas/recipe";
import {iDbRecipeCardIngredience} from "@/zodSchemas/ingredientsRecipecard";
import {insertIngredientsForRecipe} from "@/db/queries/insertIngredients";
import {insertRecipeCategory} from "@/db/queries/insertRecipeCategory";
import {deleteStep, insertSteps} from "@/db/queries/insertSteps";
...
export async function createRecipeAction(formData: FormData): Promise<CreateRecipeResult> {
    ...
    try {
        recipeId = await insertRecipe(CreateRecipeRowSchema.parse(parsedRecipe.data))
    } catch (e) {
    ...
    try {
        const {ingredients} = JSON.parse(raw) as { ingredients: iDbRecipeCardIngredience [] }
